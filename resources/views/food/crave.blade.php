                    ...
                    <div class="p-6 lg:p-8 flex flex-col justify-between">
                        <div>
                            ...
                            <div class="space-y-6 mb-6">
                                <template x-for="addonCategory in ((selectedItem && selectedItem.addon_categories && selectedItem.addon_categories.length > 0) ? selectedItem.addon_categories : defaultAddonCategories)" :key="addonCategory.id">
                                    <div class="space-y-3">
                                                ...
                                                <label class="flex items-center justify-between p-3.5 rounded-xl bg-charcoal-900/60 border border-white/5 hover:border-amber-glow/20 cursor-pointer transition-all"
                                                    ...
                                                    <div class="flex items-center gap-3.5">
                                                        ...
                                                        <div>
                                                            ...
                                                            <span class="block text-amber-warm/70 text-xs mt-0.5" x-text="'+' + '$' + Number(addon.price).toFixed(2)"></span>
                                                        </div>
                                                    </div>
                                                    <input type="checkbox"
                                                           :value="addon.id"
        ...
        function craveApp() {
            return {
                ...
                categories: (window.__INITIAL_CATEGORIES && window.__INITIAL_CATEGORIES.length > 0)
                    ? [{ id: 'all', label: { ar: 'الكل', en: 'All' } }, ...window.__INITIAL_CATEGORIES.map(c => ({
                        id: c.slug || c.id,
                        label: { ar: c.title || c.name, en: c.title || c.name }
                      }))]
                    : [{ id: 'all', label: { ar: 'الكل', en: 'All' } }],

                // Addon categories loaded from Database
                defaultAddonCategories: (window.__INITIAL_ADDON_CATEGORIES && window.__INITIAL_ADDON_CATEGORIES.length > 0)
                    ? window.__INITIAL_ADDON_CATEGORIES.map(c => ({
                        id: c.id,
                        name: { ar: c.name, en: c.name },
                        is_multiple: true,
                        addons: (c.active_addons || c.addons || []).map(a => ({
                            id: a.id,
                ...
                async fetchMenuItems() {
                    try {
                        ...
                        console.error('Failed to fetch menu items from backend:', e);
                    }
                },
